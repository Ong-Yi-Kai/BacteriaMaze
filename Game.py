...
class Game:
    ...
    def __init__(self, num_cells: int, food_pos: List[Tuple[int, int]],
        ...
        self.food_scent = Scent(self.dimx, self.dimy, food_pos, self.maze)
        ...
        for _ in range(num_cells):
            ...
            self.cells.append(Cell(self.dimx, self.dimy, Game.cell_pattern,
                                   init_pos, self.food_scent.get_scent, self.maze))
    ...
    def update(self):
        ...
        for i in range(self.dimx):
            for j in range(self.dimy):
                ...
                color = [0, 0, 0]
                if cells_grid[i, j] > 0:
                    color = [0, min(50*cells_grid[i, j], 255), 255]

                elif self.food_scent.data[i, j] > 0:
                    color = [
                        0, int(math.tanh(self.food_scent.data[i, j]) * 200), 0]
                    # color = [0,0,0]
